...
/** Races promise against a timeout; rejects with 'timed out' if exceeded. */
...
export class MiraiSender implements ISender {
  ...
  constructor(transport: MiraiTransport) { this.transport = transport; }

  /** Wait until API is alive or timeoutMs elapses. */
  ...
  /** Send text — with per-attempt timeout + retry on transient MQTT errors. */
  ...
  /**
   * Send typing indicator — best-effort with 3s timeout.
   * If sendTypingIndicator callback never fires (MQTT cycling), we resolve after 3s
   * so callers are never blocked by a hanging indicator.
   */
  async sendTyping(recipientId: string): Promise<void> {
    ...
    // 3-second max wait — never block on a hanging typing indicator
    try {
